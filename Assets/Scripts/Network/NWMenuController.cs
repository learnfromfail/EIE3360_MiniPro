...
public class NWMenuController : NetworkBehaviour {
    ...
    IEnumerator ClickWaitSupport()
    {
        ...
        while (true)
        {
            ...
            {
                BS.round++;
                WhoCurrentInTurn.GetComponentInChildren<Camera>().gameObject.GetComponent<NWCameraMovement>().setFly(BS.Characters[BS.RankWhoseTurn(BS.round + 1)].transform.position);
                goBack();
                break;
            }
            fromZero++;
